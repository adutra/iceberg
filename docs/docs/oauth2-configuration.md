...
The root URL of the Authorization server, which will be used for discovering supported endpoints and their locations.
...
Optional. Either this property or `rest.auth.oauth2.token-endpoint` must be set, unless a static token (`rest.auth.oauth2.token`) is provided.

### `rest.auth.oauth2.token-endpoint`

URL of the OAuth2 token endpoint. If not set, the token endpoint will be discovered from the issuer URL (`rest.auth.oauth2.issuer-url`), using the OpenID Connect Discovery metadata published by the issuer.

Optional. Either this property or `rest.auth.oauth2.issuer-url` must be set, unless a static token (`rest.auth.oauth2.token`) is provided.
...
Optional. Required unless a static token (`rest.auth.oauth2.token`) is provided.
...
Client secret to use when authenticating against the OAuth2 server. Optional. Rquired when the client is private and is authenticated using the standard "client-secret" methods.

### `rest.auth.oauth2.scope`

Space-separated list of scopes to include in each request to the OAuth2 server.

Optional. If absent, no scopes are sent.
...
Extra parameters to include in each request to the token endpoint. This is useful for custom parameters that are not covered by the standard OAuth2 specification. Defaults to an empty map.
...
The token acquisition timeout. Defaults to `PT5M`. The default timeout is intentionally large, in order to accommodate for long-running flows that require human intervention (e.g. Authorization Code flow).
...
Optional. If absent, defaults to `true` when the initial grant is `client_credentials`, and to `false` otherwise.
...
Defaults to `PT1H`. Must be a valid [ISO-8601 duration](https://en.wikipedia.org/wiki/ISO_8601#Durations).

### `rest.auth.oauth2.token-refresh.prefetch`

Prefetch duration to use; a new token will be fetched when the current token's remaining lifespan is less than this value. Defaults to `PT10S`. Must be a valid [ISO-8601 duration](https://en.wikipedia.org/wiki/ISO_8601#Durations).

### `rest.auth.oauth2.token-refresh.jitter`

Maximum jitter to subtract from the computed token renewal delay, to avoid thundering-herd effects when many clients refresh at the same time.

A random value within `[0, jitter)` is subtracted from the delay, so renewal is spread over the window `[expirationTime - prefetch - jitter, expirationTime - prefetch]`, and thus the prefetch duration is always respected.

Defaults to `PT5S`. Setting this property to zero disables jitter. Must be a valid [ISO-8601 duration](https://en.wikipedia.org/wiki/ISO_8601#Durations).
...
Optional. Required when the grant type is `urn:ietf:params:oauth:grant-type:token-exchange`.

### `rest.auth.oauth2.token-exchange.subject-token-type`

The type of the subject token. Must be a valid URN.

Optional. If absent, defaults to `urn:ietf:params:oauth:token-type:access_token`.
...
Optional. If absent, defaults to `urn:ietf:params:oauth:token-type:access_token` when an actor token is used.

### `rest.auth.oauth2.token-exchange.requested-token-type`

The type of the requested token. Must be a valid URN.
...
Defaults to an empty list. Can be a single value or a comma-separated list of values.
