...
public abstract class UserInteraction implements Runnable {
  ...
  /**
   * Callback to invoke when an error occurs. Allows signaling user interaction failures back to the
   * user emulator thread.
   */
  protected abstract Consumer<Throwable> errorListener();

  /** The SSL context to use for HTTPS requests. */
  protected abstract Optional<SSLContext> sslContext();
  ...
  protected URI login(URI loginPageUrl, String username, String password, Set<String> cookies)
    ...
    // receive login page
    String loginHtml = htmlPage(loginPageUrl, cookies);
  ...
  protected String htmlPage(URI url, Set<String> cookies) throws Exception {
