...
public class HydraContainer extends GenericContainer<HydraContainer> {
  ...
  private final List<HydraClient> clients = Lists.newArrayList();
  ...
  public HydraContainer withClient(String clientId, String clientSecret, String authMethod) {
    clients.add(ImmutableHydraClient.of(clientId, clientSecret, authMethod));
    return this;
  }
  ...
  private void createClient(HydraClient hydraClient, Client httpClient) {
    ...
    builder.append("],");
    builder
        .append("\"token_endpoint_auth_method\":\"")
        .append(hydraClient.authMethod())
        .append("\"");
    builder.append("}");
    ...
    try (Response response =
        httpClient.target(adminUrl()).path("/admin/clients").request().post(Entity.json(json))) {
