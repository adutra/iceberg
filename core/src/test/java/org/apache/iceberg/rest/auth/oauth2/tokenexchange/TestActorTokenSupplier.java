...
class TestActorTokenSupplier {
  ...
  private static OAuth2Config createMainConfig(
    ...
    builder.put(TokenExchangeConfig.PREFIX + TokenExchangeConfig.SUBJECT_TOKEN, "subject-token");
    builder.put(
    ...
    actorTokenConfig.forEach(
        (k, v) ->
            builder.put(TokenExchangeConfig.PREFIX + TokenExchangeConfig.ACTOR_TOKEN + '.' + k, v));

    if (actorToken != null) {
      builder.put(TokenExchangeConfig.PREFIX + TokenExchangeConfig.ACTOR_TOKEN, actorToken);
    }

    return OAuth2Config.fromProperties(builder.build());
  }

  private static ActorTokenSupplier createSupplier(OAuth2Config config) {
    ScheduledExecutorService executor = mock(ScheduledExecutorService.class);
    return ActorTokenSupplier.create(config, OAuth2ClientRuntime.of(executor));
  }
}
