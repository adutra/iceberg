...
public final class OAuth2Client implements Closeable {
  ...
  TokensResult currentTokens() {
    try {
      ...
      throw new RuntimeException("Token acquisition timed out", e);
   ...
   * lifespan to {@link TokensResult#issuedAt()} otherwise.
   */
  private Instant expirationTime(TokensResult currentTokens) {
    return currentTokens
        .accessTokenExpirationTime()
        .orElseGet(
            () -> currentTokens.issuedAt().plus(config.tokenRefreshConfig().accessTokenLifespan()));
  }
