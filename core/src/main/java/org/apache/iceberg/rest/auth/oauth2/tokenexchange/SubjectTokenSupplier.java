...
public abstract class SubjectTokenSupplier extends AbstractTokenSupplier {
  ...
  public static SubjectTokenSupplier create(OAuth2Config config, OAuth2ClientRuntime runtime) {
  ...
  protected void validate() {
    if (staticToken().isEmpty() && dynamicTokenConfig().isEmpty()) {
  ...
  protected Optional<TypelessAccessToken> staticToken() {
    return mainConfig().tokenExchangeConfig().subjectToken();
  }

  @Override
  protected TokenTypeURI staticTokenType() {
    return mainConfig().tokenExchangeConfig().subjectTokenType();
  }

  @Override
  protected Optional<OAuth2Config> dynamicTokenConfig() {
    return mainConfig().tokenExchangeConfig().subjectTokenConfig();
  }
