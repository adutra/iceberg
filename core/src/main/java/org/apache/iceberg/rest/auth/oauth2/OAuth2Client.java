 ...
 */
final class OAuth2Client implements Closeable {
  ...
  OAuth2Client(OAuth2Config cfg, OAuth2Runtime runtime) {
    config = cfg;
  ...
  private CompletionStage<TokensResult> handleRefreshResult(
    ...
    if (newTokens != null) {
      ...
      Duration prefetch = config.tokenRefreshConfig().prefetch();
      if (expirationTime.minus(prefetch).isAfter(now)) {
  ...
  private Duration nextRenewal(@Nullable TokensResult currentTokens, Instant now) {
    ...
    Duration prefetch = config.tokenRefreshConfig().prefetch();
    Duration delay = Duration.between(now, expirationTime).minus(prefetch);

    Duration jitter = config.tokenRefreshConfig().jitter();
    if (!jitter.isZero()) {
      delay = delay.minusMillis(ThreadLocalRandom.current().nextLong(jitter.toMillis()));
    }
