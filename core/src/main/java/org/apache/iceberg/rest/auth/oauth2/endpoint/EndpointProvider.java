...
public abstract class EndpointProvider {
  ...
  protected abstract HTTPRequestSender httpClient();

  @Value.Lazy
  public URI resolvedTokenEndpoint() {
    return tokenEndpoint().orElseGet(() -> openIdProviderMetadata().getTokenEndpointURI());
  }

  @Value.Lazy
  public URI resolvedAuthorizationEndpoint() {
    return authorizationEndpoint()
        .orElseGet(() -> openIdProviderMetadata().getAuthorizationEndpointURI());
  }

  @Value.Lazy
  public URI resolvedDeviceAuthorizationEndpoint() {
    return deviceAuthorizationEndpoint()
        .or(() -> Optional.ofNullable(openIdProviderMetadata().getDeviceAuthorizationEndpointURI()))
        .orElseThrow(
            () ->
  ...
  @Value.Lazy
  protected ReadOnlyAuthorizationServerEndpointMetadata openIdProviderMetadata() {
    URI issuerUrl =
  ...
  private ReadOnlyAuthorizationServerEndpointMetadata oidcProvider(Issuer issuer)
    ...
    HTTPResponse httpResponse = request.toHTTPRequest().send(httpClient());
