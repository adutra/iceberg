...
public class ApacheHttpClient implements HttpClient {
  ...
  public static HttpClientBuilder createApacheClientBuilder(HttpClientConfig config) {
    ...
    connManager.setDefaultSocketConfig(
        SocketConfig.custom()
            .setTcpNoDelay(true)
            .setSoTimeout(Timeout.of(config.readTimeout()))
            .build());
    ...
    try {
      ...
      if (config.sslTrustStorePath().isPresent()) {
        sslContextBuilder.loadTrustMaterial(
            config.sslTrustStorePath().get(),
            config.sslTrustStorePassword().map(String::toCharArray).orElse(null));
      } else if (config.sslTrustAll()) {
    ...
    connManager.setTlsSocketStrategy(
        new DefaultClientTlsStrategy(
            sslContext,
            config.sslProtocols().isEmpty()
                ? HttpsSupport.getSystemProtocols()
                : config.sslProtocols().toArray(new String[0]),
            config.sslCipherSuites().isEmpty()
                ? HttpsSupport.getSystemCipherSuits()
                : config.sslCipherSuites().toArray(new String[0]),
            SSLBufferMode.STATIC,
            config.sslHostnameVerificationEnabled()
    ...
    RequestConfig requestConfig =
        RequestConfig.custom()
            ...
            .setMaxRedirects(5)
            .setContentCompressionEnabled(config.compressionEnabled())
            .build();

    builder.setDefaultRequestConfig(requestConfig);

    if (config.proxyHost().isPresent() && config.proxyPort().isPresent()) {
      HttpHost proxy = new HttpHost(config.proxyHost().get(), config.proxyPort().getAsInt());
      ...
      if (config.proxyUsername().isPresent() && config.proxyPassword().isPresent()) {
        ...
        credentialProvider.setCredentials(
            ...
            new UsernamePasswordCredentials(
                config.proxyUsername().get(), config.proxyPassword().get().toCharArray()));
