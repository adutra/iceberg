...
public class OAuth2Manager implements AuthManager {
  ...
  public AuthSession contextualSession(SessionContext context, AuthSession parent) {
    ...
    return getOrCreateCacheById(childConfig)
        .get(
            context.sessionId(),
            id ->
                new OAuth2Session(
  ...
  public AuthSession tableSession(
    ...
    return getOrCreateCacheByConfig(childConfig)
        .get(
            childConfig,
            cfg ->
                new OAuth2Session(
  ...
  public AuthSession tableSession(RESTClient sharedClient, Map<String, String> properties) {
    ...
    return getOrCreateCacheByConfig(config)
        .get(
            config,
  ...
  private Cache<String, OAuth2Session> getOrCreateCacheById(OAuth2Config config) {
    return getOrCreateCache(cacheById, config);
  }

  private Cache<OAuth2Config, OAuth2Session> getOrCreateCacheByConfig(OAuth2Config config) {
    return getOrCreateCache(cacheByConfig, config);
  }

  private <K> Cache<K, OAuth2Session> getOrCreateCache(
      AtomicReference<Cache<K, OAuth2Session>> ref, OAuth2Config config) {
    Cache<K, OAuth2Session> cache = ref.get();
    if (cache == null) {
      ref.compareAndSet(null, newCache(config));
      cache = ref.get();
    }
    return cache;
  }
