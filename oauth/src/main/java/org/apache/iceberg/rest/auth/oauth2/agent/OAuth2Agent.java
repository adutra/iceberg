...
import org.apache.iceberg.rest.auth.oauth2.concurrent.AutoCloseables;
import org.apache.iceberg.rest.auth.oauth2.concurrent.Futures;
import org.apache.iceberg.rest.auth.oauth2.flow.FlowFactory;
import org.apache.iceberg.rest.auth.oauth2.flow.InitialFlow;
import org.apache.iceberg.rest.auth.oauth2.flow.RefreshFlow;
...
import org.apache.iceberg.rest.auth.oauth2.token.Token;
...
public final class OAuth2Agent implements Closeable {
  ...
  private static final CompletableFuture<Tokens> MUST_FETCH_NEW_TOKENS_FUTURE =
  ...
  public AccessToken authenticate() {
    return authenticateInternal().accessToken();
  }
  ...
  Tokens getCurrentTokens() {
    try {
      Duration timeout = spec.basicConfig().timeout();
  ...
  public void close() {
    if (closing.compareAndSet(false, true)) {
      try (flowFactory;
          ...
          AutoCloseables.UncheckedAutoCloseable ignored1 =
              AutoCloseables.cancelOnClose(agentAccessed);
          AutoCloseables.UncheckedAutoCloseable ignored2 =
              AutoCloseables.cancelOnClose(currentTokensFuture);
          AutoCloseables.UncheckedAutoCloseable ignored3 =
              AutoCloseables.cancelOnClose(tokenRefreshFuture)) {
    ...
    if (newTokens != null) {
      if (LOGGER.isDebugEnabled()) {
        ...
        LOGGER.debug(
            ...
            name,
            newTokens.accessToken().expirationTime());
        RefreshToken refreshToken = newTokens.refreshToken();
        LOGGER.debug(
            "[{}] Refresh token expiration time: {}",
            name,
            refreshToken == null ? "null" : refreshToken.expirationTime());
      }
    } else if (!closing.get()) {
      Throwable toWarn = error;
      if (error instanceof CompletionException) {
        toWarn = error.getCause();
      }

      if (toWarn instanceof RESTException) {
        // Don't include the stack trace if the error is a RESTException,
        // since it's not very useful and just clutters the logs.
        maybeWarn("[{}] Failed to fetch new tokens: {}", name, toWarn.toString());
      } else {
        maybeWarn("[{}] Failed to fetch new tokens", name, toWarn);
      }
    }
  }

  private void maybeScheduleTokensRenewal(@Nullable Tokens currentTokens) {
    if (!spec.tokenRefreshConfig().enabled()) {
      LOGGER.debug(
    ...
    boolean idle = timeSinceLastAccess.compareTo(spec.tokenRefreshConfig().idleTimeout()) > 0;
  ...
  private Duration nextTokenRefresh(@Nullable Tokens currentTokens, Instant now) {
    Duration minRefreshDelay = spec.tokenRefreshConfig().minRefreshDelay();
    if (currentTokens == null || currentTokens.accessToken().expired(now)) {
      return minRefreshDelay;
    }

    Token token = currentTokens.accessToken();
    Instant expirationTime = token.resolvedExpirationTime();
    if (expirationTime == null) {
      Duration defaultLifespan = spec.tokenRefreshConfig().accessTokenLifespan();
      maybeWarn(
    ...
    Duration delay =
        Duration.between(now, expirationTime).minus(spec.tokenRefreshConfig().safetyMargin());
  ...
  private void wakeUp(Instant now) {
    ...
    if (currentTokens == null || isRisky(currentTokens.accessToken(), now)) {
  ...
  private boolean isRisky(@Nullable Token token, Instant now) {
    return token == null || token.expired(now.plus(spec.tokenRefreshConfig().safetyMargin()));
  }
