...
public abstract class SubjectTokenSupplier extends AbstractTokenSupplier {
  ...
  protected void validate() {
    if (token().isEmpty() && agentConfig().isEmpty()) {
  ...
  protected Optional<String> token() {
    return mainSpec().tokenExchangeConfig().subjectToken();
  }

  @Override
  protected URI tokenType() {
    return mainSpec().tokenExchangeConfig().subjectTokenType();
  }

  @Override
  protected Map<String, String> agentConfig() {
    return mainSpec().tokenExchangeConfig().subjectTokenConfig();
  }

  @Override
  protected String defaultAgentName() {
    return mainSpec().runtimeConfig().agentName() + "-subject";
  }
}
