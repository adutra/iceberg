...
public interface AuthorizationCodeConfig {
  ...
  String DEFAULT_CALLBACK_BIND_HOST = "localhost";

  AuthorizationCodeConfig DEFAULT = builder().build();

  /**
   * The OAuth2 authorization endpoint. Either this or {@link BasicConfig#issuerUrl()} must be set,
   * if the grant type is {@link GrantType#AUTHORIZATION_CODE}.
   *
   ...
   */
  @Value.Default
  default String callbackBindHost() {
   ...
   */
  @Value.Default
  default PkceTransformation pkceTransformation() {
  ...
  default void validate() {
    ...
    if (callbackBindPort().isPresent()) {
      validator.check(
          ...
          OAuth2Properties.AuthorizationCode.CALLBACK_BIND_PORT,
  ...
  default AuthorizationCodeConfig merge(Map<String, String> properties) {
    ...
    builder.endpointOption().set(properties, authorizationEndpoint());
    builder.redirectUriOption().set(properties, redirectUri());
    builder.callbackBindHostOption().set(properties, callbackBindHost());
    builder.callbackBindPortOption().set(properties, callbackBindPort().stream().boxed().findAny());
    builder.callbackContextPathOption().set(properties, callbackContextPath());
    builder.pkceEnabledOption().set(properties, pkceEnabled());
    builder.pkceTransformationOption().set(properties, pkceTransformation());
  ...
  interface Builder {
    ...
    private ConfigOption<URI> endpointOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.ENDPOINT, this::authorizationEndpoint, URI::create);
    }

    private ConfigOption<URI> redirectUriOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.REDIRECT_URI, this::redirectUri, URI::create);
    }

    private ConfigOption<String> callbackBindHostOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.CALLBACK_BIND_HOST, this::callbackBindHost);
    }

    private ConfigOption<Integer> callbackBindPortOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.CALLBACK_BIND_PORT,
          this::callbackBindPort,
          Integer::parseInt);
    }

    private ConfigOption<String> callbackContextPathOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.CALLBACK_CONTEXT_PATH, this::callbackContextPath);
    }

    private ConfigOption<Boolean> pkceEnabledOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.PKCE_ENABLED,
          this::pkceEnabled,
    ...
    private ConfigOption<PkceTransformation> pkceTransformationOption() {
      return ConfigOptions.simple(
          OAuth2Properties.AuthorizationCode.PKCE_TRANSFORMATION,
          this::pkceTransformation,
