...
public abstract class TokenExchangeRequest implements TokenRequest {
  ...
  public final Map<String, String> asFormParameters() {
    ...
    if (resource != null) {
      data.put(RESOURCE, resource.toString());
    }
    ...
    if (requestedTokenType != null) {
      data.put(REQUESTED_TOKEN_TYPE, requestedTokenType.toString());
    }

    data.put(SUBJECT_TOKEN, subjectToken());
    data.put(SUBJECT_TOKEN_TYPE, subjectTokenType().toString());
    if (actorToken() != null) {
      data.put(ACTOR_TOKEN, actorToken());
    }

    URI actorTokenType = actorTokenType();
    if (actorTokenType != null) {
      data.put(ACTOR_TOKEN_TYPE, actorTokenType.toString());
    }
