...
import static org.apache.iceberg.rest.auth.oauth2.test.TestConstants.NOW;
...
class TestToken {
  ...
  @ParameterizedTest
  @MethodSource("expiredTestCases")
  void testExpired(Token token, Instant now, boolean expected) {
    assertThat(token.expired(now)).isEqualTo(expected);
  }

  static Stream<Arguments> expiredTestCases() {
    return Stream.of(
