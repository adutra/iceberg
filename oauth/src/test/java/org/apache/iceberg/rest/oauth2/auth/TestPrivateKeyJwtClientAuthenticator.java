...
class TestPrivateKeyJwtClientAuthenticator {
  ...
  void authenticate(
    ...
    assertThat(authenticator.tokenEndpoint()).isEqualTo(URI.create("https://example.com/token"));
    assertThat(authenticator.clientAssertionConfig()).isEqualTo(clientAssertionConfig);
    ...
    assertThat(request.clientAssertion()).satisfies(requirements);
  }
        ...
        Arguments.of(
            ...
            (Consumer<TypedToken>)
                assertion -> {
                  ...
                  assertThat(assertion.tokenType()).isEqualTo(TypedToken.URN_JWT_BEARER);
                  DecodedJWT jwt = JWT.decode(assertion.payload());
