...
import org.apache.iceberg.rest.oauth2.rest.ImmutableClientCredentialsTokenRequest;
...
public abstract class IcebergClientCredentialsExpectation extends InitialTokenFetchExpectation {
  ...
  protected PostFormRequest tokenRequestBody() {
    return ImmutableClientCredentialsTokenRequest.builder()
        .clientId(String.format("(%s|%s)", TestConstants.CLIENT_ID1, TestConstants.CLIENT_ID2))
        .clientSecret(
            String.format("(%s|%s)", TestConstants.CLIENT_SECRET1, TestConstants.CLIENT_SECRET2))
  ...
  protected void addRequestHeaders(HttpRequest request) {
    // Iceberg client credentials does not require any special headers
  }
}
