...
import org.apache.iceberg.rest.oauth2.OAuth2Manager;
import org.apache.iceberg.rest.oauth2.OAuth2Properties.Basic;
import org.apache.iceberg.rest.oauth2.OAuth2Properties.Runtime;
...
import org.apache.iceberg.rest.oauth2.test.expectation.ImmutableLoadTableEndpointExpectation;
...
import org.apache.iceberg.rest.oauth2.test.server.InactiveHttpServer;
...
@Value.Immutable(copy = false) // class is too big
...
public abstract class TestEnvironment implements AutoCloseable {
  ...
  public HttpServer server() {
    return unitTest() ? new MockHttpServer() : new InactiveHttpServer();
  }
  ...
  public Map<String, String> subjectTokenConfig() {
    ImmutableMap.Builder<String, String> builder =
        ImmutableMap.<String, String>builder()
            .put(Basic.GRANT_TYPE, subjectGrantType().commonName())
            .put(Basic.CLIENT_ID, subjectClientId())
            .put(Basic.CLIENT_SECRET, subjectClientSecret())
            .put(Basic.EXTRA_PARAMS_PREFIX + "extra2", "value2");
    ConfigUtils.scopesAsString(subjectScopes()).ifPresent(scope -> builder.put(Basic.SCOPE, scope));
    return builder.build();
  }
  ...
  public Map<String, String> actorTokenConfig() {
    ImmutableMap.Builder<String, String> builder =
        ImmutableMap.<String, String>builder()
            .put(Basic.GRANT_TYPE, actorGrantType().commonName())
            .put(Basic.CLIENT_ID, actorClientId())
            .put(Basic.CLIENT_SECRET, actorClientSecret())
            .put(Basic.EXTRA_PARAMS_PREFIX + "extra2", "value2");
    ConfigUtils.scopesAsString(actorScopes()).ifPresent(scope -> builder.put(Basic.SCOPE, scope));
    return builder.build();
  }
    ...
    if (forceInactiveUser()) {
      ...
      GrantType subjectGrant =
          mainGrant == GrantType.TOKEN_EXCHANGE
                  ...
                  && tokenExchangeConfig().subjectTokenConfig().containsKey(Basic.GRANT_TYPE)
              ? GrantType.fromConfigName(
                  tokenExchangeConfig().subjectTokenConfig().get(Basic.GRANT_TYPE))
              : GrantType.CLIENT_CREDENTIALS;
      GrantType actorGrant =
          mainGrant == GrantType.TOKEN_EXCHANGE
                  && tokenExchangeConfig().actorToken().isEmpty()
                  && tokenExchangeConfig().actorTokenConfig().containsKey(Basic.GRANT_TYPE)
              ? GrantType.fromConfigName(
                  tokenExchangeConfig().actorTokenConfig().get(Basic.GRANT_TYPE))
              : GrantType.CLIENT_CREDENTIALS;
  ...
  public Map<String, String> catalogProperties() {
    return ImmutableMap.<String, String>builder()
        ...
        .put(Basic.GRANT_TYPE, grantType().toString())
        .put(Basic.ISSUER_URL, authorizationServerUrl().toString())
        .put(Basic.CLIENT_ID, clientId())
        .put(Basic.CLIENT_SECRET, clientSecret())
        .put(Basic.SCOPE, ConfigUtils.scopesAsString(scopes()).orElse(TestConstants.SCOPE1))
        .put(Basic.DIALECT, dialect().toString())
        .put(Basic.EXTRA_PARAMS_PREFIX + "extra1", "value1")
        .put(Runtime.AGENT_NAME, agentName())
        .build();
  }
